...
export function MermaidDiagram({
  ...
  const renderDiagram = useCallback(async () => {
    ...
    try {
      ...
      cleanCode = cleanCode.replace(/^```mermaid\s*/i, "").replace(/```$/i, "").trim();
      cleanCode = cleanCode.replace(/\r\n/g, "\n");
      if (!/^\s*(flowchart|graph|sequenceDiagram|classDiagram|stateDiagram|erDiagram|journey|gantt)\b/i.test(cleanCode)) {
      ...
      // Keep Mermaid's rendered colors intact. Diagram sanitization is handled upstream.
      setSvg(renderedSvg);
    } catch (err) {
