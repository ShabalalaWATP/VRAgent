...
| **[Dynamic Analysis](docs/DYNAMIC_ANALYSIS_README.md)** | Runtime and network security testing | PCAP deep inspection, ZAP, Nuclei, Nmap, MITM proxy, API testing, fuzzing tools |
...
## PCAP Analyzer Highlights

- Deep Inspection panels expose attack surface, HTTP sessions, WebSocket payloads, TCP stream previews, database traffic, extracted files, and timeline evidence from supported captures.
- The built-in AI chat can answer follow-up questions using the analyzed capture and persisted deep-inspection artifacts in saved reports.
- Plaintext previews appear when traffic is already cleartext or can be reconstructed from the capture. TLS, HTTPS, and WSS payloads are not decrypted automatically.
